#!/usr/bin/env python3
"""
UBUNTU20-STIG V2R3 Python Implementation - AIR-GAP EDITION
===========================================================
...
NEW IN V3.0.0 - AIR-GAP EDITION:
[OK] 100% offline operation - no internet required
[OK] Installs packages from local .deb files
[OK] Automatic detection of offline package directory
[OK] Intelligent fallback to system package cache
[OK] Full remote execution capability via SSH
...
Author: Converted from DISA STIG V2R3 XCCDF + Air-Gap Enhancements
License: MIT
Version: 3.0.0 (Air-Gap Edition - 100% Offline Operation)
"""
...
class PackageManager(SystemModifier):
    """Manage package installation and removal - AIR-GAP EDITION"""

    def __init__(self):
        super().__init__()
        self.offline_mode = os.environ.get('STIG_OFFLINE_MODE', 'false').lower() == 'true'
        self.offline_pkg_dir = "/tmp/stig_ubuntu_packages"

        # Check if offline packages are available
        if os.path.exists(self.offline_pkg_dir):
            deb_count = len(list(Path(self.offline_pkg_dir).glob("*.deb")))
            if deb_count > 0:
                logger.info(f"Offline mode: Found {deb_count} .deb files in {self.offline_pkg_dir}")
                self.offline_mode = True

    def update_cache(self) -> bool:
        """Update apt package cache"""
        if self.offline_mode:
            logger.info("Offline mode: Skipping apt cache update")
            return True
    ...
    def install_packages(self, packages: List[str]) -> bool:
        """Install required packages - supports both online and offline mode"""
        logger.info(f"Installing packages: {', '.join(packages)}")

        if self.offline_mode:
            return self._install_packages_offline(packages)
        else:
            return self._install_packages_online(packages)

    def _install_packages_online(self, packages: List[str]) -> bool:
        """Install packages using apt-get (online mode)"""
        success = True
    ...
    def _install_packages_offline(self, packages: List[str]) -> bool:
        """Install packages from local .deb files (offline mode)"""
        logger.info(f"Offline mode: Installing from {self.offline_pkg_dir}")

        if not os.path.exists(self.offline_pkg_dir):
            logger.warning(f"Offline package directory not found: {self.offline_pkg_dir}")
            logger.warning("Attempting to install from system cache...")
            return self._install_packages_online(packages)
        ...
        for package in packages:
            ...
            if self.is_package_installed(package):
                logger.info(f"[OK] Already installed: {package}")
                success_count += 1
                continue

            # Find .deb file for this package
            deb_files = list(Path(self.offline_pkg_dir).glob(f"{package}_*.deb"))
            if not deb_files:
                # Try with package name prefix
                deb_files = list(Path(self.offline_pkg_dir).glob(f"{package}*.deb"))

            if deb_files:
                deb_file = str(deb_files[0])
                logger.info(f"Installing from: {Path(deb_file).name}")
                ...
                if result:
                    self.changes.append(f"Installed package (offline): {package}")
                    logger.info(f"[OK] Installed: {package}")
                    success_count += 1
                else:
                    # Try to fix dependencies
                    logger.warning(f"dpkg install failed, trying to fix dependencies...")
                    self.run_command(['apt-get', '-f', 'install', '-y'], check=False)

                    # Retry installation
                    result2, _ = self.run_command(['dpkg', '-i', deb_file], check=False)
                    if result2:
                        self.changes.append(f"Installed package (offline): {package}")
                        logger.info(f"[OK] Installed: {package} (after dependency fix)")
                        success_count += 1
                    else:
                        logger.warning(f"[FAIL] Failed to install: {package}")
                        failed_packages.append(package)
            else:
                logger.warning(f"No .deb file found for: {package}")
                failed_packages.append(package)

        # If some packages failed, try installing all .deb files in the directory
        if failed_packages:
            logger.info("Attempting batch installation of all .deb files...")
            all_debs = list(Path(self.offline_pkg_dir).glob("*.deb"))
            if all_debs:
                deb_list = ' '.join([str(d) for d in all_debs])
                result, _ = self.run_command(
                    ['dpkg', '-i'] + [str(d) for d in all_debs],
                    check=False
                )
                # Fix dependencies
                self.run_command(['apt-get', '-f', 'install', '-y'], check=False)

                # Re-check failed packages
                for package in failed_packages[:]:
                    if self.is_package_installed(package):
                        logger.info(f"[OK] Installed via batch: {package}")
                        success_count += 1
                        failed_packages.remove(package)

        if failed_packages:
            logger.warning(f"Failed to install {len(failed_packages)} packages: {', '.join(failed_packages)}")
            logger.warning("These packages may need to be provided or are not critical")

        logger.info(f"Package installation: {success_count}/{len(packages)} successful")

        # Return True if at least some packages were installed
